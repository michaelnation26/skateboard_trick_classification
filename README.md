...
![kickflip](readme_content/50-50.gif) ![360 kickflip](readme_content/360_kickflip.gif)
...
A two-stream model (RGB+audio) was trained to classify skateboard tricks from video clips. Deep Mind's RGB-I3D model was used for the RGB stream. For the audio stream, spectrogram images were first generated to represent the audio. Then, a CNN extracted the features from the spectrogram.
...
* fail
​
![grind](readme_content/spectrogram.jpg)
...
The weights of the RGB-I3D model used in this RGB+audio model are initialized with the weights pre-trained on ImageNet and Kinetics datasets. i.e. The weights from the trained 1. RGB Model were not used to initialize the weights in this RGB+audio model.
...
fail
Dataset
Same preprocessing as described in 1. RGB Model and 2. Audio Model.
